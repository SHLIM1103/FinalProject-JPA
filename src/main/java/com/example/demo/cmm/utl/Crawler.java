...
public class Crawler {
	public static void main(String[] args) {
		...
		try {
			Document rawData = Jsoup.connect("https://thepicker.net/BATHROOM").timeout(10000).get();
			...
			Elements prdImgHover = rawData.select("img[class=_hover_img hover_img]");
			...
			for(int i = 0; i < prdName.size(); i++) {
				...
				product.setPrdImgHover(prdImgHover.get(i).attr("abs:alt-src"));
			...
			for (ProductDto prd : prdBox) {
				...
				Cell prdImgHoverCell = row.createCell(1);
				prdImgHoverCell.setCellValue(prd.getPrdImgHover());
				
				Cell priceCell = row.createCell(2);
				priceCell.setCellValue(prd.getPrice());
			}
			
			try (FileOutputStream outputStream = new FileOutputStream("C:\\Users\\SOHYUN LIM\\Developer Liiim\\03bathroom02.csv")) {
